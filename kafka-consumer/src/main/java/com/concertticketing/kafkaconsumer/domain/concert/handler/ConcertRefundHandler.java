...
import com.concertticketing.commonavro.ConcertRefundEvent;
...
import com.concertticketing.kafkaconsumer.common.handler.manual.ConsumerRetryManualHandler;
...
public class ConcertRefundHandler
    implements ConsumerRetryManualHandler<ConcertRefundEvent> {
    ...
    @Override
    @KafkaListener(
        topics = {CONCERT_REFUND},
        groupId = CONCERT_REFUND_RETRY,
        containerFactory = SINGLE_MANUAL_ACK_CONSUMER_FACTORY
    )
    public void consume(ConcertRefundEvent event, Acknowledgment ack) {
        try {
            handler(event, ack);
    ...
    @Override
    public void handler(ConcertRefundEvent event, Acknowledgment ack) {
        ...
        if (!concertSeatReservation.canRefund()) {
    ...
    @Override
    public void handleRetry(ConcertRefundEvent event, Acknowledgment ack, Exception e) {
        kafkaProducer.send(
            CONCERT_REFUND,
            new ConcertRefundEvent(
                event.getEventId(),
                event.getConcertId(),
                event.getUserId(),
                event.getScheduleId(),
